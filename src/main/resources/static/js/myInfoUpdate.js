...
$( function(){
	...
	$("#btnUpdate").on("click", function(){
		...
		if(!existCheckNickname){
			...
			updateParams.u_nickname = u_nickname.val();
		}
		...
		if(u_name.val().replace(/^\s*/, "") == null || u_name.val().replace(/^\s*/, "") == ""){
			...
			updateParams.u_nickname = u_nickname.val().replace(/^\s*/, "");
		}
		...
		if(!emailRegex.test(u_email.val())){
			...
			updateParams.u_email = u_email.val().replace(/^\s*/, "");
		}
		...
		if(u_birth.val() == null || u_birth.val() == ""){
			...
			updateParams.u_birth = u_birth.val();
		}
		
		if(u_gender_m.is(":checked")){
			updateParams.u_gender = u_gender_m.val();	
		}else{
			updateParams.u_gender = u_gender_w.val();
		}
		
		updateParams.u_phone = u_phone.val();
		updateParams.u_postcode = u_postcode.val();
		updateParams.u_address = u_address.val();
	...
	$("#btnUserDelete").on("click", function(){
		let params = {
			u_seq : $("#u_seq").val(),
			u_pw : $("#u_pw_delete").val()
		}
		
		$.ajax({
			url : "/user/userDeleteProcess",
			data : params,
			type : "post",
			success : function(returnData){
				if(returnData == true){
